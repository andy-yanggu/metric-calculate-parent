...
- 数据明细宽表：是对输入的明细数据进行一个抽象、映射和定义。数据明细宽表类似SQL中的表，有名称和多个字段。

- 宽表字段：是数据明细宽表中的字段，分为普通字段和时间字段。包含了对字段的名称、数据类型的定义以及注释。

- 时间字段：数据明细宽表中某个字段或者多个字段的业务语义是时间，可以标识其为时间字段且定义时间格式。数据明细宽表必须定义时间字段，且时间字段可以定义多个。

- 前置过滤条件：对输入的数据进行过滤，类似于SQL中的WHERE。一段字符串（本质上是Aviator表达式），输入数据是明细数据，返回值是Boolean。

- 维度字段：将维度进行抽取出来，统一进行管理。分为字段维度和业务维度。

- 绑定维度：类似于SQL中的GROUP BY分组字段，可以定义多个分组字段，将宽表字段和维度字段进行绑定。

- 时间聚合粒度：选择数据明细宽表中定义的时间字段，同时定义时间聚合单位（秒、分、时、天、月、年）和长度。

- 聚合函数：数值型（SUM、AVG、MAX等）、对象型（MAXOBJECT等）、集合型（DISTINCTLIST）、映射型（BASEMAP）、滑动计数窗口（过去N笔）、混合型、CEP类型
...
SELECT
	...
	SUM(amount) AS sum_amount //SUM聚合逻辑, amount就是计算表达式, sum_amount指标名称
FROM 
...
聚合函数分为：数值型、对象型、集合型、映射型、混合型、CEP类型

| 聚合类型 | 配置数据                                       | 输出数据          | 说明                           |
| :------- | :--------------------------------------------- | ----------------- | ------------------------------ |
| 数值型   | 计算结果为数值的表达式                         | 数值              |                                |
| 对象型   | 比较字段列表、保留字段和原始数据               | 对象（JSON对象）  |                                |
| 集合型   | 去重字段列表、排序字段列表、保留字段和原始数据 | 集合（List和Set） | 滑动计数窗口需要额外聚合处理器 |
| 映射型   | key生成字段，value配置聚合逻辑                 | 映射（Map）       |                                |
| 混合型   | 多个聚合逻辑和多个聚合逻辑的计算表达式         | -                 | 多个基本聚合类型               |
| CEP类型  | 类似Flink的CEP功能                             | -                 | 需要额外聚合处理器             |
...
- 滑动计数（COUNTWINDOW）: 最近5笔交易的聚合值: 同一账号过去30天中最近5笔交易的总金额 √
...
- 配置：排序表达式列表（List<FieldOrderParam> ）
- 输入：明细数据（JSONObject）
...
对于聚合类型的字段处理器输入的都是明细数据，输出的都是MergedUnit。

#### 5.1 数值型、对象型和集合型（基本聚合类型）

- 数值型：通过表达式计算得到度量值，对度量值进行聚合运算。需要MetricFieldProcessor（计算得到度量值）
...
- 基本聚合类型：BaseAggregateFieldProcessor（数值型、集合型、对象型）
...
#### 5.2 映射型（底层是Map）
   ...
   通过多字段去重字段处理器得到多字段去重列表作为map的key，value是基本聚合类型（BaseAggregateFieldProcessor）。
...
一种特殊的集合型，输出的数据就是List<JSONObject>，进行二次聚合。需要额外聚合处理

BaseUdafParam baseUdafParam和BaseUdafParam externalBaseUdafParam
