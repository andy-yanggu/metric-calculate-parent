...
A给B转账，有转出账号、转入账号、交易金额和时间戳字段，形成交易流水数据。

计算指标：转入账号一天（自然天）中的累计转入金额（交易金额大于1才统计）。
...
- 定义数据明细宽表（trade_detail）（转账交易流水明细表）
  - 配置宽表字段
    - account_no_out：转出账号，字符串类型
    - account_no_in：转入账号，字符串类型
