...
- 数据明细宽表：是对输入的明细数据进行一个抽象、映射和定义。数据明细宽表类似SQL中的表，有名称和多个字段，包含了对字段的名称、数据类型的定义以及注释。

- 宽表字段：是数据明细宽表中的字段，分为普通字段和时间字段。

- 时间字段：数据明细宽表中某个字段或者多个字段的业务语义是时间，可以标识其为时间字段且包含时间格式。数据明细宽表必须定义时间字段，且时间字段可以定义多个。
...
```sql
SELECT
    account_no_in AS account_no, //account_no_in宽表字段, account_no维度字段，这个过程成为绑定维度
