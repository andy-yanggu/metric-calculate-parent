...
- 数据明细宽表：是对输入的明细数据进行一个抽象、映射和定义。数据明细宽表类似SQL中的表，有名称和多个字段。一般输入的都是JSON字符串。
...
- 前置过滤条件：对输入的数据进行过滤，类似SQL语句中的WHERE。一段字符串（本质上是Aviator表达式），输入数据是明细数据，返回值是Boolean。前置过滤条件表达式可以由宽表中的任意字段进行组合编写表达式，返回boolean即可。

- 维度字段：将维度进行抽取出来，统一进行管理。分为字典维度和业务维度。

- 绑定维度：类似于SQL中的GROUP BY分组字段，可以定义多个分组字段，将宽表字段和维度字段进行绑定。

- 窗口切分：数据是源源不断的，常用各种方式将数据分桶。常用窗口切分方式有时间窗口、状态窗口、计数窗口、会话窗口、全窗口、事件窗口。
...
A给B转账，有转出账号（account_no_out）、转入账号（account_no_in）、转账金额（amount）和时间戳字段（trans_timestamp），形成交易流水数据（trade_detail）。
