...
聚合函数分为：数值型、对象型、集合型、映射型、滑动计数窗口、混合型、CEP类型
...
- 基本映射（BASEMAP）: 同一账户过去30天所有交易账号的累计交易金额topN的交易账号列表 √
...
对象型（对表达式的值进行比较，输出的是对象）

集合型（对表达式的值进行排序、去重，输出的是集合）

映射型（对表达式的值进行分组生成key，然后value的值进行聚合，输出的是Map）
...
2. 各种聚合、排序、分组、比较的字段都是表达式。例如IF(type = '张三', amount, 0)。
3. 排序可以进行多表达式进行排序。例如ORDER BY (age + 1) ACS, age DESC。
4. 去重可以进行多表达式去重。
5. 限制长度的逻辑和SQL的LIMIT语义一致。
6. 混合类型，一般是只同类型的基本聚合计算，进行运算。
7. 映射类型的value类型是基本聚合类型。
8. 滑动计数窗口和CEP需要外部聚合字段处理器，也是基本聚合类型。
