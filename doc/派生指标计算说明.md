...
5. 在反射调用空参构造方法实例化聚合函数之后会反射给聚合字段的成员变量赋值，数据来源于聚合函数中的param参数（本质上是一个Map），map的key是成员变量的字段名。
6. 给成员变量反射赋值后会调用init()方法进行初始化。init方法只会调用一次。

#### 3.3 聚合函数三个泛型说明

1. IN表示输入的度量值。例如SUM，输入的是Number类型的数值。
2. ACC表示累加器，中间变量。例如SUM，中间变量为Double，默认为0.0D。
...
聚合函数注解主要用于确定聚合函数唯一标识，描述信息，聚合函数类型，需要从明细中提取的数据，以及标识聚合函数参数。
...
#### 10.3 无状态-计算接口（内存攒批查询）

#### 10.4 有状态-计算接口（多线程）

#### 10.5 有状态-计算接口（批处理）

#### 10.6 有状态-计算接口（内存攒批查询和攒批更新）
