...
public class GlobalExceptionHandler {
    ...
    public Result<Void> handlerBusinessException(BusinessException exception, HttpServletRequest request) {
        log.warn("[业务异常] 请求url: {}, 请求方式: {}, 异常code: {}, 异常信息: {}", request.getRequestURI(),
                request.getMethod(), exception.getCode(), exception.getMessage(), exception);
        return Result.fail(exception.getCode(), exception.getMessage());
    }

    @ExceptionHandler(Exception.class)
    public Result<Void> handlerException(Exception exception, HttpServletRequest request) {
        log.error("[运行时异常] 请求url: {}, 请求方式: {}, 异常信息: {}", request.getRequestURI(), request.getMethod(),
