...
public class ListUnit<T extends Cloneable<T>> implements UnlimitedMergedUnit<ListUnit<T>>,
    ...
    @Deprecated
    public ListUnit<T> merge(com.yanggu.metric_calculate.core.unit.collection.ListUnit<T> that) {
        return merge(that, false);
    }

    private ListUnit<T> merge(com.yanggu.metric_calculate.core.unit.collection.ListUnit<T> that, boolean useLimit) {
        if (that == null) {
            return this;
        }
